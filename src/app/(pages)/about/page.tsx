...
interface SocialLinks {
  github: string;
  linkedin: string;
  twitter?: string; // Made optional with the ? modifier
}
...
export default function AboutPage() {
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 relative bg-white dark:bg-gray-900 transition-colors duration-300">
      {/* Background elements */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-5 transition-opacity duration-300">
        <div className="absolute inset-0 bg-[url('/images/grid.svg')] bg-center bg-cover dark:opacity-100 opacity-100"></div>
      </div>
      ...
      <motion.div 
        ...
        >
          ...
          <motion.div variants={itemVariants}>
            <h1 className="text-4xl font-bold mb-2 text-gray-900 dark:text-white transition-colors duration-300">{aboutData.name}</h1>
            <div className="flex items-center mb-4">
              <span className="text-pink-400 font-mono mr-2">$</span>
              <p className="text-xl text-indigo-400 font-mono">{aboutData.title}</p>
            </div>
            <p className="text-gray-600 dark:text-gray-300 mb-6 font-mono transition-colors duration-300">{aboutData.bio}</p>
        ...
        <motion.div className="mb-16" variants={itemVariants}>
          <motion.h2 
            className="text-2xl font-bold mb-6 text-gray-900 dark:text-white border-b border-indigo-500/30 pb-2"
            variants={itemVariants}
          >
            About Me
          </motion.h2>
          <motion.div 
            ...
            variants={itemVariants}
          >
            <div className="text-gray-700 dark:text-gray-300 font-mono leading-relaxed" dangerouslySetInnerHTML={{ __html: aboutData.longBio }} />
          </motion.div>
        </motion.div>
                ...
                <div key={index} className="mb-8">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {skillGroup.technologies.map((tech, techIndex) => (
                      <span 
                        key={techIndex} 
                        className="px-3 py-1 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-full text-sm font-mono"
                      >
                        {tech}
                      </span>
                    ))}
